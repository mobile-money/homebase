module.exports = function(app, Summary) {
	...
	app.get("/api/v1/money/summaries", function(req, res) {
		...
		Summary.getAll().then(function(results) {
			if (results.length > 0) {
	...
	app.get("/api/v1/money/summaries/:id", function(req, res) {
		...
		Summary.getByAccountId(Number(req.params.id)).then(function(results) {
			if (results.length > 0) {
	...
	app.get("/api/v1/money/unique/summaries", function(req, res) {
		...
		Summary.getAllUnique().then(function(results) {
    ...
    app.get("/api/v1/money/dataXfer/summaries/:start/:max",function(req,res) {
        Summary.dataXfer(Number(req.params.start),Number(req.params.max)).then(function(result) {
